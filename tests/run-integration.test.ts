 ...
 *
 * Returns: { binDir, verifyScriptPath, prCreateArgsLog }
 */
...
}): { binDir: string; verifyScriptPath: string; prCreateArgsLog: string } {
  const {
    implementExitCode = 0,
    verifyStatus = "ok",
    prCreateExitCode = 0,
...
`
  ...
  return { binDir, verifyScriptPath, prCreateArgsLog };
}
 ...
 */
function writeFakeConfig(tmpDir: string, binDir: string, verifyScriptPath: string): string {
...
describe("runCommand integration", () => {
  test("(a) implement-failure outcome skips push and PR creation", async () => {
    ...
    const { binDir, verifyScriptPath, prCreateArgsLog } = makeFakeBin(tmpBinDir, {
    ...
    const configPath = writeFakeConfig(tmpBinDir, binDir, verifyScriptPath);
  ...
  test("(b) PrError from createPr propagates out of runCommand", async () => {
    ...
    const { binDir, verifyScriptPath } = makeFakeBin(tmpBinDir, {
      implementExitCode: 0,
      verifyStatus: "ok",
